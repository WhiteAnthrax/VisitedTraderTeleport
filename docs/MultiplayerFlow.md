...
クライアントがトレーダーと話してから、行き先へ移動する／行き先を消すまでの時間軸。矢印はすべて実際のネットパッケージ。
...
sequenceDiagram
    ...
    P->>C: 行き先を1つ選ぶ
    C-->>P: 移動する / この行き先を忘れる / 戻る

    Note over P,F: 3-a. 移動する
    P->>C: 「移動する」
    ...
    Note over P,F: 3-b. 行き先を忘れる
    P->>C: 「この行き先を忘れる」→ 確認
    C->>S: ForgetRequest（行き先キー）
    S->>S: 送信元からプレイヤーを特定
    S->>F: そのプレイヤーの訪問記録だけ削除
    S-->>C: 結果のメッセージ
    S-->>C: Snapshot（更新された一覧）
    C-->>P: 一覧から消える
```
...
| 245 | ForgetRequest | C → S | この行き先の自分の訪問記録を消したい |
